...
function Sidebar({page,pageId}) {
 ...
 const EMAIL = await axios({
  ...
  function IconContainerModal(props) {
    const { value, ...other } = props;
    return <span {...other}>{customIconsModal[value].icon}</span>;
  }
...
const [cPhone,setCPhone]= useState('');
  useEffect(() => {
    ...
    <div style={{marginTop: '20px'}}>
        ...
        <div onClick={() => history.push(`/vandore/${pageId}/about`)}>
        ...
        <SidebarRow Icon={Chat} title="Chat With Us" page={page}/>
                ...
                {show ? channels.map(channel => (
                    <div onClick={() => history.push(`/chatsPublic/${pageId}/${channel.id}`)} page={page}>
          ...
          <SidebarRow Icon={Storefront} title="Store" page={page}/>
           ...
           {site_settings.photoGallery ? (
                   <div onClick={() => history.push(`/vandore/${pageId}/gallery`)} >
 ...
 >
    ...
    {openRate ? '' : (
 <div className='external' style={{marginTop: '20px'}}>
 {zomatoLink==='' ? '' : (
